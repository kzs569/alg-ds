...
public class DoubleArrayTrie {
    ...
    public void Insert(String s) throws Exception
    {
        ...
        {
            //获取状态位置
            cur_position = Base[pre_position]+GetCharCode(s.charAt(i));
            //如果长度超过现有，拓展数组
            if (cur_position >= Base.length) Extend_Array();

            //空闲状态
            if(Base[cur_position] == 0 && Check[cur_position] == 0){
                Base[cur_position] = -Pos;
                Check[cur_position] = pre_position;
                Pos = CopyToTailArray(s,i+1);
                break;
            }//已存在状态
            else if(Base[cur_position] > 0 && Check[cur_position] == pre_position){
                pre_position = cur_position;
                continue;
            }
            //冲突 1：遇到 Base[cur_p]小于0的，即遇到一个被压缩存到Tail中的字符串
            else if(Base[cur_position] < 0 && Check[cur_position] == pre_position){
                int head = -Base[cur_position];
                //插入重复字符串
                if(s.charAt(i+1)== END_CHAR && Tail[head]==END_CHAR){break;}
                ...
                if (Tail[head] == s.charAt(i+1)){
                    ...
                    Base[cur_position] = avail_base;

                    Check[avail_base+GetCharCode(s.charAt(i+1))] = cur_position;
                    ...
                    Check[avail_base+GetCharCode(Tail[head])] = cur_position;
                    Check[avail_base+GetCharCode(s.charAt(i+1))] = cur_position;
            ...
            else if(Check[cur_position] != pre_position){
                ArrayList<Integer> list1 = GetChildList(pre_position);
                int toBeAdjust;
                ArrayList<Integer> list = null;
                if(true)
                {
                    toBeAdjust = pre_position;
                    list = list1;
                }
